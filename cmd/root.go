...
https://dns (and in turn https://github.com/miekg/dns) for constructing
...
func init() {
	...
	// rabbitmq
	rootCmd.PersistentFlags().StringVar(&GC.RabbitmqConfigPath, "rabbitmq-file", "none", "the file that contains rabbitmq server config")
	...
	rootCmd.PersistentFlags().BoolVar(&GC.HTTPSEnable, "enable-https", false, "Perform lookups over HTTPS")
	rootCmd.PersistentFlags().BoolVar(&GC.HTTPSGetEnable, "https-get", false, "determine https method, default post")
	...
	rootCmd.PersistentFlags().BoolVar(&GC.TLSEnable, "enable-tls", false, "Perform lookups over TLS")

	rootCmd.PersistentFlags().StringVar(&GC.HttpsPath, "enable-https-path", "/dns-query", "Perform lookups over HTTPS")
